		...
		else if(!GameRules_GetProp("m_bInWaitingForPlayers", 1))
		{
			...
			if(bosses > 0)
			{
				...
				if(count)
				{
					...
					{
						count = Preference_GetBossQueue(players, MaxClients, 0, team);
			...
			// Standard FF2
			int boss[1];
			if(Preference_GetBossQueue(boss, 1, 0))
			{
				int team;
...
static Action Gamemode_IntroTimer(Handle timer)
{
	for(int client = 1; client <= MaxClients; client++)
	{
		...
		{
			if(!Client(client).IsBoss || (!Bosses_PlaySoundToClient(client, client, "sound_intro", .volume = SNDVOL_BOSS) && !Bosses_PlaySoundToClient(client, client, "sound_begin", .volume = SNDVOL_BOSS)))
			{
...
void Gamemode_RoundEnd(int winteam)
{
	...
	*/
	...
	int color[4];
	TF2Tools_GetTeamColor4(winner, color);
	...
	for(int a; a < total; a++)
	{
		...
		{
			float pos = 0.5 - (Configs_TeamCount() * 0.05);
